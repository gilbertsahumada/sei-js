...
export function registerTokenTools(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ limit = 20, minPrice = 0 }) => {
      console.log("🔧[TOOL_get_dragonswap_prices]");
      try {
  ...
  server.tool(
    ...
    async ({ query, limit = 10 }) => {
      console.log("🔧[TOOL_search_dragonswap_tokens]");
      try {
        if (!shouldFetchProtocolData("dragonswap")) {
          return {
            content: [{
  ...
  server.tool(
    "get_sailor_prices",
    "Get Sailor Finance tokens with real-time prices - USE ONLY when user specifically asks for Sailor Finance prices or market data. DO NOT use for arbitrage analysis.",
    {
      limit: z.number().optional().describe("Max number of tokens to return (default: 20)"),
      minPrice: z.number().optional().describe("Minimum price in USD (default: 0)"),
    ...
    async ({ limit = 20, minPrice = 0, debug = false }) => {
      console.log("🔧[TOOL_get_sailor_prices]");
      try {
  ...
  server.tool(
    ...
    "Get token prices from Sailor Finance API with daily changes. ALWAYS check the address you are querying it exists in Sailor Finance",
    {
  ...
  server.tool(
    "get_yaka_prices",
    "Get Yaka Finance tokens with real-time prices - USE ONLY when user specifically asks for Yaka Finance prices or market data. DO NOT use for arbitrage analysis.",
    {
      limit: z.number().optional().describe("Max number of tokens to return (default: 20)"),
      minPrice: z.number().optional().describe("Minimum price in USD (default: 0)")
    },
